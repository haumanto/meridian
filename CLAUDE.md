...
| confidenceSizingEnabled / confidenceFirstDeployMult / confidenceFloorMult / confidenceFullWinRate / confidenceMinSamples | management | false / 0.7 / 0.5 / 60 / 3 — "start small, grow with evidence": deploy size × multiplier from the pool's adjusted win-rate. Unproven/new pool (samples < min) → 0.7×; ≥min samples & adjWR ≥ 60 → full; ≥min but weak → 0.5×. Default off = no change. Composes with the vol-band clamp (both only shrink). `confidence-sizing.js` |
